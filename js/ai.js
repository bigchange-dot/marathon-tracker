...
const AI = (() => {
  ...
  function runLine(r) {
    ...
    const parts = [
      ...
      r.trialSec ? '5km 전력 구간 ' + fmtDur(r.trialSec) + ' → ' + trialTier(r.trialSec).goal + ' 트랙' : null,
  ...
  function buildPrompt(run) {
    ...
    const lines = [
      ...
      '',
      '## 러너 배경',
      '- 하프 마라톤 기록 2:14:33 (6\'22"/km, 2026년 4월) — 전력이 아닌 목표 페이스 주행. 이후 3개월 공백, 7/27 재개',
      '- 10km 56:00 (5\'36"/km, 2025년 11월) — 오버페이스로 뛴 전력 기록. 풀코스 환산 상한 4:28~4:30',
      '- 9/18 5km 기록 측정으로 대회 목표 트랙 결정: 27:30 이내 4:30 / 28:30 이내 4:40 / 그 외 4:48~4:55',
      ...
      '- 14주 플랜(8/11~11/15, 주 구분 화→월): 주 3~4회 — 수 회복 · 금 퀄리티 · 일 보조 조깅 · 월 롱런(9/7부터, 주말 롱런 불가로 이동). 피크 주간 55km, 최장 롱런 28km. 롱런·대회는 9분 런 / 1분 워크.',
      '',
      '## 방금 끝난 훈련',
      '- ' + runLine(run),
    ];

    if (t) lines.push(`- 이 타입(${t.label})의 목표 페이스존(/km): ${paceText(run.type)} — ${t.desc}` + (t.runWalk ? ' 기록 페이스는 워크 포함 평균.' : ''));
    if (run.shoes) {
